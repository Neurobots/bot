...
module RubyCounter
  ...
  def unbind
    ...
    EventMachine.add_timer(10){ $spoolerHandler = EventMachine.popen("./neuroBot.rb", RubyCounter) }
    uptime = `date +%s`.to_i
    $spoolerPid = -1
...
EM.run {
   Dir.chdir("/home/joshua/neuroserver/bot")
   puts `pwd`
   $spoolerHandler = EventMachine.popen("./neuroBot.rb", RubyCounter) 
   uptime = `date +%s`.to_i
