module Songended
    ...
    self.client.on :song_ended do |song|
      ...
      if song.played_by != self.client.user
                self.db.query("insert into bot_sstats_#{MAGICKEY} set songid='#{digest(song.title, song.artist)}', title='#{self.db.escape_string(song.title)}', artist='#{self.db.escape_string(song.artist)}', first_user_played='#{song.played_by.id}' on duplicate key update times_played=times_played")
